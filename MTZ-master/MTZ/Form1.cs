...
namespace MTZ
{
    public partial class Form1 : Form
    {
        ...
        static List<MSG> IterateMessages(Folder folder, string monat, string year, string name)
        {
            ...
            var fi = folder.Items;
            bool notFound = false;
            string shortMonat = monat.Substring(0, 3);
            if (fi != null)
            {
                foreach (var item in fi)
                {
                    ...
                    try
                    {
                        ...
                        if (mi.Subject.Contains("Stempelzeiten") && (mi.Subject.Contains(monat) || mi.Subject.Contains(shortMonat)) && mi.ReceivedTime.Year == Int16.Parse(year) /*&& (mi.Subject.Contains(year) || mi.Subject.Contains(yearWithLastTwoDigits))*/)
                        {
                        ...
                        if (mi.Subject.Contains("Stempelzeiten") && (mi.Subject.Contains(monat) || mi.Subject.Contains(shortMonat)))
                        {
